...
function HighlightViewer({
  ...
  const goPrevSlide = () => {
    if (slideIdx > 0) {
      ...
      const prevHl = highlights[hlIdx - 1];
      setHlIdx(h => h - 1);
      setSlideIdx(prevHl.slides.length - 1);
    }
  };
  ...
  useEffect(() => {
    if (paused) return;
    timerRef.current = setTimeout(goNextSlide, 3500);
    return () => { if (timerRef.current) clearTimeout(timerRef.current); };
  }, [hlIdx, slideIdx, paused]);

  // Reset slide when highlight changes
  useEffect(() => { setSlideIdx(0); }, [hlIdx]);
    ...
    <div className="fixed inset-0 z-[100] bg-black/95 flex items-center justify-center" onClick={onClose}>
      ...
      >
        ...
        <div className="absolute top-3 left-3 right-3 z-20 flex gap-1">
          {hl.slides.map((_, i) => (
            <div key={i} className="flex-1 h-0.5 rounded-full bg-white/30 overflow-hidden">
              <div
                className={`h-full rounded-full bg-white ${i === slideIdx && !paused ? "animate-none" : ""}`}
                style={{
        ...
        <div className={`w-full h-full bg-gradient-to-br ${hl.color} flex flex-col items-center justify-center gap-5 px-8`}>
          ...
          <p className="text-white font-bold text-center text-base leading-snug relative z-10 drop-shadow">
            {hl.slides[slideIdx].text}
          </p>
        ...
        {(hlIdx > 0 || slideIdx > 0) && (
          <button onClick={goPrevSlide}
            className="absolute left-2 top-1/2 -translate-y-1/2 z-30 w-9 h-9 rounded-full bg-black/40 hover:bg-black/60 backdrop-blur-sm flex items-center justify-center text-white transition-all">
            <ChevronLeft size={18} />
          </button>
        )}
        <button onClick={goNextSlide}
          className="absolute right-2 top-1/2 -translate-y-1/2 z-30 w-9 h-9 rounded-full bg-black/40 hover:bg-black/60 backdrop-blur-sm flex items-center justify-center text-white transition-all">
          <ChevronRight size={18} />
        </button>

        {/* Highlight switcher dots */}
        <div className="absolute bottom-4 left-0 right-0 z-20 flex justify-center gap-1.5">
          {highlights.map((_, i) => (
            <button key={i} onClick={() => { setHlIdx(i); setSlideIdx(0); }}
              className={`w-1.5 h-1.5 rounded-full transition-all ${i === hlIdx ? "bg-white w-4" : "bg-white/40"}`} />
          ))}
        </div>
      </div>
    </div>
...
function AddHighlightModal({ onClose }: { onClose: () => void }) {
  ...
  const inputCls = "w-full px-3 py-2 bg-gray-800 border border-gray-700 rounded-lg text-sm text-white placeholder-gray-500 focus:outline-none focus:border-red-500";
  return (
    <div className="fixed inset-0 z-50 bg-black/80 backdrop-blur-sm flex items-center justify-center p-4" onClick={onClose}>
      <div className="bg-gray-900 border border-gray-700 rounded-2xl w-full max-w-sm overflow-hidden shadow-2xl" onClick={e => e.stopPropagation()}>
        ...
        <div className="p-5 space-y-4">
          <div>
            <label className="block text-xs font-bold text-gray-400 uppercase tracking-wider mb-1.5">Highlight Name</label>
            <input className={inputCls} placeholder="e.g. Cheerdance, Esports..." value={name} onChange={e => setName(e.target.value)} />
          </div>
          <div>
            <label className="block text-xs font-bold text-gray-400 uppercase tracking-wider mb-1.5">Cover Photo</label>
...
function UploadModal({ onClose, onAdd }: { onClose: () => void; onAdd: (type: "post" | "reel", item: any) => void }) {
  ...
  const SPORTS = ["Basketball", "Volleyball", "Swimming", "Track & Field", "Badminton", "Opening Ceremony", "Awards", "Other"];
  const TAGS = step === "post"
    ? ["FINALS", "SEMI-FINALS", "QUARTERFINALS", "RECORD BROKEN", "OPENING", "CLOSING", "DAY 1", "DAY 2", "DAY 3"]
    : ["HIGHLIGHTS", "COMPILATION", "BEST MOMENTS", "BEHIND THE SCENES", "CEREMONY"];

  const inputCls = "w-full px-3 py-2 bg-gray-800 border border-gray-700 rounded-lg text-sm text-white placeholder-gray-500 focus:outline-none focus:border-red-500 focus:ring-1 focus:ring-red-500/20";
          ...
          {step === "choose" && (
            <div className="space-y-3">
              <button onClick={() => setStep("post")}
                className="w-full flex items-center gap-4 p-4 bg-gray-800 hover:bg-gray-750 border border-gray-700 hover:border-red-500/50 rounded-xl transition-all group text-left">
            ...
            <div className="space-y-4">
              ...
              {step === "post" ? (
                <>
                  <div>
                    ...
                    <textarea className={`${inputCls} resize-none`} rows={3} placeholder="Write a caption..." value={caption} onChange={e => setCaption(e.target.value)} />
                  </div>
                ...
                <div>
                  ...
                  <input className={inputCls} placeholder="e.g. Best Dunks Compilation" value={title} onChange={e => setTitle(e.target.value)} />
                </div>
              )}
              ...
              <div className="flex gap-3 pt-1">
                ...
                <button onClick={handleSubmit}
                  className="flex-1 px-4 py-2.5 bg-[#A91D3A] hover:bg-[#8B1528] text-white rounded-lg text-sm font-bold transition-colors shadow-sm">
...
// ─── Post Detail Modal (Facebook/IG style floating post) ──────────────────────
      ...
      <div className="bg-gray-900 border border-gray-700 rounded-2xl w-full max-w-lg overflow-hidden shadow-2xl max-h-[90vh] flex flex-col" onClick={e => e.stopPropagation()}>
        ...
        <div className="p-4 overflow-y-auto flex-1">
          ...
          <div className="flex items-center gap-2 border-t border-gray-800 pt-4">
            ...
            <input
              className="flex-1 bg-gray-800 border border-gray-700 rounded-full px-4 py-1.5 text-sm text-white placeholder-gray-500 focus:outline-none focus:border-red-500"
    ...
    <div className="min-h-screen bg-gray-950 font-sans text-white">
      ...
      <div className="bg-gray-900 border-b border-gray-800 px-6 py-5">
        <div className="flex items-center justify-between gap-4 flex-wrap">
          ...
          {/* Stats — posts only, no fake followers */}
