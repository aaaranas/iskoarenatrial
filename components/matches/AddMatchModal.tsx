...
const SPORTS = ["Badminton", "Basketball", "Volleyball", "Table Tennis", "Football"];
const CATEGORIES = ["Men's Singles", "Men's Doubles", "Women's Singles", "Women's Doubles", "Mixed Doubles"];
const HOME_COLLEGES = ["COE", "CBA", "CAS"];
const AWAY_COLLEGES = ["CON", "CHE", "CCS"];
...
const EMPTY_FORM = {
  ...
  awayTeam: "",
  location: "",
  statusType: "upcoming" as Match["statusType"],
};
...
export const AddMatchModal = ({ children, onAdd }: AddMatchModalProps) => {
  ...
  const handleSubmit = () => {
    if (!form.league || !form.homeTeam || !form.awayTeam) return;
    ...
    onAdd({
      ...
      homeTeamShort: form.homeTeam.slice(0, 3).toUpperCase(),
      awayTeam: form.awayTeam,
      awayTeamShort: form.awayTeam.slice(0, 3).toUpperCase(),
      homeScore: null,
      awayScore: null,
      minute: null,
      isOwner: true,
          ...
          <div className="space-y-8">
            ...
            <div className="grid grid-cols-2 gap-8">
              <div className="space-y-4">
                ...
                <Select value={form.league} onValueChange={(val) => handleField("league", val)}>
              ...
              <div className="space-y-4">
                ...
                <Select value={form.category} onValueChange={(val) => handleField("category", val)}>
                  <SelectTrigger className="bg-zinc-900/50 border-zinc-800 focus:ring-[#C5A059]/20 text-xs font-bold tracking-widest uppercase">
              ...
              <div className="grid grid-cols-11 items-center gap-4">
                <div className="col-span-5 space-y-3">
                  <Input
                    ...
                    value={form.homeTeam}
                    onChange={(e) => handleField("homeTeam", e.target.value)}
                    className="bg-transparent border-b border-zinc-800 border-t-0 border-x-0 rounded-none focus-visible:ring-0 focus-visible:border-[#C5A059] uppercase text-xs tracking-widest font-black placeholder:text-zinc-700"
                  />
                  <div className="flex gap-2">
                    {HOME_COLLEGES.map((c) => (
                      <span
                        key={c}
                        onClick={() => handleField("homeTeam", c)}
                        className="px-2 py-0.5 border border-zinc-800 text-[8px] font-bold text-zinc-600 rounded hover:text-[#C5A059] hover:border-[#C5A059]/50 cursor-pointer transition-all"
                      >
                        {c}
                      </span>
                    ))}
                ...
                <div className="col-span-5 space-y-3">
                  <Input
                    ...
                    value={form.awayTeam}
                    onChange={(e) => handleField("awayTeam", e.target.value)}
                    className="bg-transparent border-b border-zinc-800 border-t-0 border-x-0 rounded-none focus-visible:ring-0 focus-visible:border-[#C5A059] text-right uppercase text-xs tracking-widest font-black placeholder:text-zinc-700"
                  />
                  <div className="flex gap-2 justify-end">
                    {AWAY_COLLEGES.map((c) => (
                      <span
                        key={c}
                        onClick={() => handleField("awayTeam", c)}
                        className="px-2 py-0.5 border border-zinc-800 text-[8px] font-bold text-zinc-600 rounded hover:text-[#C5A059] hover:border-[#C5A059]/50 cursor-pointer transition-all"
                      >
                        {c}
                      </span>
              ...
              <div className="space-y-4">
                ...
                <div className="relative">
                  ...
                  <Input
                    ...
                    className="bg-zinc-900/50 border-zinc-800 pl-10 h-10 text-[10px] font-bold tracking-widest uppercase focus-visible:ring-[#C5A059]/20"
                  />
                </div>
              </div>
            </div>
            ...
            <div className="pt-6 flex justify-end gap-4 border-t border-white/5">
              ...
              <button
                ...
                className="px-8 py-2 bg-[#C5A059] text-black text-[10px] font-black uppercase tracking-[0.2em] hover:bg-[#D4B475] transition-all shadow-[0_0_20px_rgba(197,160,89,0.2)]"
              >
                Confirm Entry
              </button>
            </div>
