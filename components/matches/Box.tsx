...
interface BoxProps {
  /** When provided by IskoArena/MatchesPage, use this data directly. */
  matches?: Match[];
  onAddMatch?: (match: Omit<Match, "id" | "createdAt">) => void;
  onDeleteMatch?: (id: number) => void;
  /** Allow admin actions when true (defaults to false when props-driven). */
  isAdmin?: boolean;
}

export const Box = ({ matches: propMatches, onAddMatch, onDeleteMatch, isAdmin = false }: BoxProps) => {
  const matches: Match[] = propMatches ?? [];
  ...
  const filteredMatches = useMemo(() => {
    return matches.filter((m) => {
      ...
      const matchesStatus =
        filters.status === "Status" || m.statusType === filters.status.toLowerCase();
  ...
  const FilterDropdown = ({
    ...
    <DropdownMenu>
      ...
      <DropdownMenuContent
        align="start"
        className="bg-[#0A0A0A] border-zinc-800 text-zinc-300 min-w-[160px] rounded-sm shadow-2xl z-[110]"
      >
      ...
      <div className="relative z-10 pt-[35vh]">
        ...
        <motion.div className="max-w-[1600px] mx-auto mb-12 px-6 flex items-center justify-between gap-10">
          ...
          <div className="flex items-center gap-4 shrink-0">
            {isAdmin && onAddMatch && (
              <AddMatchModal onAdd={onAddMatch}>
        ...
        <section className="max-w-[1600px] mx-auto px-6 mb-16">
          ...
          <p className="text-zinc-400 max-w-2xl text-sm leading-relaxed">
            A live-updated catalog of the intramural landscape. Featuring every active entry and
            future showdown across all campus divisions.
          </p>
        </section>
        ...
        {myMatches.length > 0 && (
          <section className="mb-24 max-w-[1600px] mx-auto px-6">
            <div className="flex items-center gap-6 mb-8">
              <h2 className="text-sm font-black uppercase tracking-[0.3em] text-[#C5A059]">
                Initialized Entries
              </h2>
